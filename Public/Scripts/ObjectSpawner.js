...
function getComponentsInSceneObjects(sceneObjects, name) {
    ...
    for (var j in sceneObjects) {
        var c = sceneObjects[j].getComponents(name);
...
function getTonsByYear(year) {
  return Math.exp(-105.6443 + 0.0549 * year);
}
...
function getFrequency() {
  ...
  // Get tons of plastic
  var currentTons = getTonsByYear(year);

  // Scale by difficulty factor
  var nextUpdateTime = DIFFICULTY_FACTOR / currentTons;

  if (nextUpdateTime < MINIMUM_PLASTIC_FREQUENCY) {
    print(
      "Game running for " +
        getTime() +
        " seconds, next update = MINIMUM_PLASTIC_FREQUENCY"
    );

    return MINIMUM_PLASTIC_FREQUENCY;
  }
